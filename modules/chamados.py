...
def carregar_meus_chamados(setor):
    return run_query("""
        SELECT protocolo, tipo_inconsistencia, empresa, status, prioridade,
...
def carregar_todos_chamados():
    return run_query("""
        SELECT protocolo, setor, tipo_inconsistencia, empresa, status, prioridade,
               nome_parceiro, numero_nota, aberto_em, solicitante
...
def carregar_mensagens(protocolo):
    return run_query("""
        SELECT autor, perfil, mensagem, enviado_em
        FROM mensagens WHERE chamado_protocolo=%s
        ORDER BY enviado_em ASC
    """, (protocolo,), fetch=True)

def enviar_mensagem(protocolo, autor, perfil, mensagem):
    run_query("""
        INSERT INTO mensagens (chamado_protocolo, autor, perfil, mensagem, enviado_em)
        VALUES (%s, %s, %s, %s, %s)
    ...
    if not mensagens:
        ...
        for autor, perfil, mensagem, enviado_em in mensagens:
            ...
            <div style='display:flex;justify-content:{alinha};margin-bottom:10px;'>
                <div style='max-width:75%;background:{bg};color:{cor_txt};
                border-radius:{border_r};padding:10px 14px;
