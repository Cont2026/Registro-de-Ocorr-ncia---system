...
def tela_admin():
    ...
    with aba[0]:
        ...
        for uid, nome, login, perfil, ativo in usuarios:
            with st.expander(f"{'🟢' if ativo else '🔴'} {'👑' if perfil=='contabilidade' else '🏢'} {nome} — `{login}`"):
                c1, c2 = st.columns(2)
                nova_senha = c1.text_input("Nova senha", key=f"s_{uid}", placeholder="Deixe em branco para não alterar")
                novo_ativo = c2.selectbox("Status", [1,0], index=0 if ativo else 1, format_func=lambda x: "Ativo" if x==1 else "Inativo", key=f"a_{uid}")
        ...
        with st.form("form_setor"):
            ...
            if st.form_submit_button("➕ Adicionar", use_container_width=True):
                if not nome.strip() or not login.strip() or not senha.strip():
                    ...
                    try:
                        run_query("INSERT INTO usuarios (nome,login,senha,perfil) VALUES (%s,%s,%s,'setor')", (nome.strip(), login.strip(), senha.strip()))
    ...
    with aba[1]:
        ...
        for i, item in enumerate(st.session_state.lista_tipos):
            ...
            with c1:
                novo_nome = st.text_input(f"Tipo {i+1}", value=item["nome"], key=f"tipo_edit_{i}_{item.get('id','novo')}", label_visibility="collapsed")
